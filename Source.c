...
GLfloat mat_specular3[] = { 1.0, 0.0, 0.0 };
GLfloat mat_ambient3[] = { 1.0, 0.0, 0.0 };
...
void DrawTextXY(double x, double y, double z, double scale, char* s)
{
    ...
    for (i = 0; i < strlen(s); i++)
        glutStrokeCharacter(GLUT_STROKE_ROMAN, s[i]); //renders a stroke character using opengl without using display list
    glPopMatrix();
}
...
#endif
    glColor3f(1.0, 0.0, 0.0);//torso blue color
...
void MechHip(char solid)
{
    ...
    SetMaterial(mat_specular, mat_ambient, mat_diffuse, mat_shininess);
#endif
    glColor3f(1.0, 0.0, 0.0);//hip lines form green
    ...
    for (i = 0; i < 2; i++) {
        ...
        SetMaterial(mat_specular2, mat_ambient2, mat_diffuse2, mat_shininess2);
#endif
        glColor3f(1.0, 0.0, 0.0);//hip line form green
        if (!solid)
...
void Shoulder(char solid)
{
    ...
    SetMaterial(mat_specular, mat_ambient, mat_diffuse, mat_shininess);
#endif
    glColor3f(1.0, 0.0, 0.0);//sholder color green
...
void UpperArm(char solid)
{
    ...
    SetMaterial(mat_specular, mat_ambient, mat_diffuse, mat_shininess);
#endif
    glColor3f(1.0, 0.0, 0.0);// arm joint green
    glRotatef(-90.0, 1.0, 0.0, 0.0);
...
void VulcanGun(char solid)
{
    ...
    SetMaterial(mat_specular2, mat_ambient2, mat_diffuse2, mat_shininess2);
#endif
    glColor3f(1.0, 0.0, 0.0);//gun color
...
void UpperLeg(char solid)
{
    ...
    SetMaterial(mat_specular, mat_ambient, mat_diffuse, mat_shininess);
#endif
    glColor3f(1.0, 0.0, 0.0);//color yellow
    if (!solid) {
        gluQuadricDrawStyle(qobj, GLU_LINE);
    }
    ...
    SetMaterial(mat_specular, mat_ambient, mat_diffuse, mat_shininess);
#endif
    glColor3f(1.0, 0.0, 0.0);//above the leg joint n below the fore leg
    glRotatef(-90.0, 1.0, 0.0, 0.0);
...
void LowerLeg(char solid)
{
    ...
    for (k = 0.0; k < 2.0; k++) {
        for (l = 0.0; l < 2.0; l++) {
            ...
            SetMaterial(mat_specular, mat_ambient, mat_diffuse, mat_shininess);
#endif
            glColor3f(1.0, 0.0, 0.0);// leg n foot joints color
            Box(1.0, 0.5, 1.0, solid);
            if (!k && !l) {
                int j;
...
void RocketPod(char solid)
{
    ...
    SetMaterial(mat_specular2, mat_ambient2, mat_diffuse2, mat_shininess2);
#endif
    glColor3f(1.0, 0.0, 0.0);//rocket port color
...
void Enviro(char solid)
{
    ...
    SetMaterial(mat_specular4, mat_ambient4, mat_diffuse4, mat_shininess4);
    glColor3f(1.0, 0.0, 0.0);//out line of the walking path
        ...
        if (frame >= 0.0 && frame <= 21.0) {
            ...
            if (step == 0) {
                ...
                if (frame > 0)
                    ...
                    hip21 = 0;
                ankle2 = -hip21;
                shoulder1 = frame * 0.25;
                shoulder2 = -frame * 0.25;
                elbow1 = frame;
                elbow2 = -frame;
            }
            else {
...
int main(int argc, char** argv)
{
    ...
    glutCreateWindow("CG PROJECT: Robot Simulation");
    myinit();
